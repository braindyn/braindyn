...
## Finally eliminate extreme values and scale for csurf
...
mri_em_register -mask $SUBJECTS_DIR/$sub-synth/mri/brainmask.mgz $datadir/$sub-tal-trunc-scale.mgz $FREESURFER_HOME/average/RB_all_2008-03-26.gca $SUBJECTS_DIR/$sub-synth/mri/transforms/talairach.lta
...
mri_surf2surf --s $sub-synth --sval-xyz white --reg $regfwd $datadir/$hires --tval-xyz --tval white.hires --hemi lh
mri_surf2surf --s $sub-synth --sval-xyz white --reg $regfwd $datadir/$hires --tval-xyz --tval white.hires --hemi rh
mri_surf2surf --s $sub-synth --sval-xyz pial  --reg $regfwd $datadir/$hires --tval-xyz --tval pial.hires  --hemi lh
mri_surf2surf --s $sub-synth --sval-xyz pial  --reg $regfwd $datadir/$hires --tval-xyz --tval pial.hires  --hemi rh
...
for vol in wm filled brain aseg
do
    mri_vol2vol \
        --mov $datadir/$hires \
        --targ $mridir/$vol.mgz \
...
lh_coords=`fsl5.0-fslstats $mridir/aseg.hires.nii.gz -l 1  -u 3  -c`
rh_coords=`fsl5.0-fslstats $mridir/aseg.hires.nii.gz -l 40 -u 42 -c`
...
do
    ...
    do
        mri_surf2surf --s $sub-synth --sval-xyz $surf.hires.deformed --reg-inv $regfwd $mridir/orig.mgz --tval-xyz --tval $surf.lores.deformed --hemi $hemi
    done
done
...
## Finally run the end bit of recon-all to get measurements etc
recon-all -subjid $sub-synth -surfvolume -parcstats -cortparc2 -parcstats2 -cortribbon -segstats -aparc2aseg -wmparc -balabels -label-exvivo-ec
