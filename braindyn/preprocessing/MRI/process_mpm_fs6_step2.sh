...
#
# USAGE:
#    process_mpm_fs6_step2.sh <subject_id> <data_directory>
#
# INPUTS:
...
recon-all -subjid $sub-synth -rmneck -skull-lta -calabel -normalization2 -maskbfs -segmentation -fill -tessellate   -smooth1 -inflate1 -qsphere -fix -white -smooth2 -inflate2 -sphere -surfreg -jacobian_white -avgcurv -cortparc -pial -openmp 3
...
recon-all -subjid $sub-synth -cortribbon -parcstats -cortparc2 -parcstats2 -cortparc3 -parcstats3 -pctsurfcon -hyporelabel -aparc2aseg -apas2aseg -segstats -wmparc -balabels -openmp 3

## Finally re-run with T2
if [[ -f $datadir/${t2vol} ]]; then
    recon-all -subject $sub-synth -T2 $datadir/$t2vol -T2pial -autorecon3 -openmp 3
fi
