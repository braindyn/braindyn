...
if params.convert_dicom
    ...
    matlabbatch{batch_idx}.spm.util.import.dicom.convopts.meta=[];
...
if params.align_to_headcast_t1
    ...
    for f=1:size(files,1)
        filename=deblank(files(f,:));
        if strcmp(filename,'MT.nii')
            matlabbatch{batch_idx}.spm.spatial.coreg.estimate.source = {fullfile(align_dir,[filename ',1'])};
        elseif strcmp(filename,'R1.nii') || strcmp(filename,'PDw.nii') || strcmp(filename,'R2s.nii') || strcmp(filename,'A.nii')
